# Dataiku Copilot Skill

**GitHub Copilot** integration for Dataiku DSS — powered by the
[dataiku_factory](https://github.com/hhobin/dataiku_factory) MCP server.

Gives GitHub Copilot in VS Code live, tool-based access to your Dataiku projects:
create/run/debug recipes, build datasets, manage scenarios, explore project flows, and more.
...
# 2. Configure your DSS connection
notepad .env
#  Set: DSS_HOST=https://your-dss.company.com:10000
#       DSS_API_KEY=your-api-key-here
#       DSS_INSECURE_TLS=true   # if using a self-signed cert

# 3. Open the workspace in VS Code
code .
```

When you open VS Code the `dataiku-factory` MCP server starts automatically.
Open Copilot Chat (`Ctrl+Alt+I`) and the Dataiku tools are ready.
...
VS Code will prompt for your `DSS_HOST`, `DSS_API_KEY`, and TLS preference on first start.
To persist those values, edit `.env` directly and update the `env` block in `mcp.json`:

```json
"env": {
  "DSS_HOST": "https://your-dss.company.com:10000",
  "DSS_API_KEY": "your-api-key-here",
...
## Using the `dataiku` Prompt

A reusable prompt file is installed at:

```
%APPDATA%\Code\User\prompts\dataiku.prompt.md
```

Use it in Copilot Chat by typing `/dataiku` (use `/` to browse all prompts).
It pre-loads all 34 Dataiku tools and a step-by-step workflow.

---

## Copilot Instructions

`.github/copilot-instructions.md` is loaded automatically by Copilot in this workspace.
It teaches Copilot all 34 MCP tools, Dataiku Python conventions, and safety rules.
...
| Category | Tools |
|----------|-------|
| Recipes | create, update, delete, run, get_code, validate_syntax, dry_run |
| Datasets | create, update, delete, build, schema, metrics, sample |
| Scenarios | create, update, delete, run, triggers, logs, steps, clone |
| Exploration | project_flow, search_objects |
| Environment | code_environments, project_variables, connections |
| Monitoring | recent_runs, job_details, cancel_jobs |
| Productivity | duplicate_structure, export_config, batch_update |
...
│   └── tools/                   ← 9 tool modules (recipes, datasets, ...)
├── scripts/
│   └── mcp_server.py             ← Server entry point (stdio / SSE)
├── .env.sample                   ← Connection config template
├── pyproject.toml
...
- Store credentials in `.env` — never commit it (already in `.gitignore`).
- Use `DSS_INSECURE_TLS=false` in production when using a trusted certificate.

---

## Credits

MCP server by [hhobin/dataiku_factory](https://github.com/hhobin/dataiku_factory).
Copilot skill layer added on top.
