...
# The agent that does the actual work. mCwnNmE2 is "Mugisha AI structured" --
# the hardened graph (routing, forced glossary lookup, read-only toolset).
# qZisMtc4 is the old free-running tools agent: pointing here bypasses every
# guardrail built this week, so change this only deliberately.
INNER_AGENT_ID = "agent:mCwnNmE2"
...
class CostReportingAgent(BaseLLM):
    ...
    def process(self, query, settings, trace):
        ...
        # Check success BEFORE touching .text: the text property calls
        # _fail_unless_success() and RAISES on a failed turn, so reading it
        # first turns every inner failure into a wrapper crash instead of a
        # passed-through error message. (Verified against dataikuapi 14.7.3.)
        if not response.success:
            err = None
            try:
                err = response._raw.get("errorMessage")
            except Exception:
                pass
            return {"text": err or "The agent was unable to produce a response."}
